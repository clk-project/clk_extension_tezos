...
def get_passphrase(passphrase: PassphraseInput = None,
                   alias: Optional[str] = None) -> bytes:
    if passphrase is None:
        passphrase = env.get('PYTEZOS_PASSPHRASE')
    if passphrase is None:
        passphrase = getpass(
            f'Please enter passphrase{" for key `" + alias + "`" if alias else ""}:\n'
        )
    if passphrase is None:
    ...
    if not isinstance(passphrase, bytes):
        raise Exception(
            f'bytes or str expected, got {type(passphrase).__name__}')
    return passphrase
...
class Key(metaclass=InlineDocstring):
    ...
    def from_secret_exponent(
        ...
        # P256
            ...
            pk = fastecdsa.keys.get_public_key(bytes_to_int(secret_exponent),
                                               curve=fastecdsa.curve.P256)
            public_point = fastecdsa.encoding.sec1.SEC1Encoder.encode_public_key(
                pk)
        else:
            assert False
    ...
    def from_mnemonic(cls,
                      ...
                      curve: bytes = b'ed',
                      activation_code: Optional[str] = None,
                      language: str = DEFAULT_LANGUAGE) -> 'Key':
    ...
    def from_faucet(cls, source: Union[str, dict]) -> 'Key':
        ...
        key = cls.from_mnemonic(mnemonic=data['mnemonic'],
    ...
    def sign(self, message: Union[str, bytes], generic: bool = False):
        ...
        if not self.secret_exponent:
            import base64
            print(base64.b64encode(encoded_message).decode())
            import sys
            signature = sys.stdin.readline().strip()
            signature = base64.b64decode(signature)
        # Ed25519
        elif self.curve == b"ed":
